...
function show(b::Ptr{Bam1_t})
    record = unsafe_load(b)
    string = bytestring(record.data)
...
function sam_read1!(samfile,bamheader,b)
    r = ccall((:sam_read1,"libhts"),Cint,(Ptr{Void},Ptr{Void},Ptr{Bam1_t}),samfile,bamheader,b)
